...
5. **Design is VERBATIM — you are not permitted to design anything.** `apps/web` serves the Claude Design bundle byte-for-byte: `apps/web/index.html` and `apps/web/support.js` are exact copies of `design/`, enforced by `scripts/check-design-verbatim.sh` (run it in CI; a failing cmp means someone touched the design — revert). Never edit those two files; when Claude Design ships an updated bundle, replace `design/` and run `pnpm --filter web sync-design`. API wiring happens ONLY through injection at serve time (see `vite.config.ts` — the runtime's `window.__resources` hook is the pattern) or a future data-adapter layer that feeds the same render paths; markup, styles, and copy are never modified. If a screen, state, or component does not exist in the bundle, STOP and record it in `docs/DESIGN_GAP_REPORT.md` for Claude Design to produce — never improvise it.
