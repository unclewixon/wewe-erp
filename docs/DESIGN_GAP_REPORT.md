...
## Phase 1.16 — closed, each verified by clicking the control

All five items below were driven through the real UI against the deployed app, not by calling hooks.

- **40. Document viewer pagination and zoom — CLOSED by Design.** Real `dvPage`/`dvZoom` state drives the arrows, the indicator, the page header and the canvas padding. Verified: › › took it 3 → 5, ‹ back to 4, + to 125%, − − to 75%. The four serve-time substitutions that stripped `onClick="{{ noop }}"` from those buttons are removed from `vite.config.ts` — their anchors no longer exist, and a `replace()` that matches nothing fails **silently**, so stale transforms are invisible rather than loud. Re-run the anchor check on every bundle.
- **41. Budget import — CLOSED.** The file input existed all along but was never appended to the document, so it could not open a dialog; Design fixed that, and `UploadBudget` now arrives with `{ fiscalYear, name, mime, dataBase64 }`. Wired to `POST /v1/budgets/import`. Verified end to end with a three-row CSV containing one deliberately malformed row: *"Imported 2 budget lines into draft v9 — 1 row was rejected and recorded in the audit log. Review before activating."* The import lands as a DRAFT; a spreadsheet is not an approval.
- **42. Signature settings — CLOSED.** The dialog existed only on `/documents/esign` with decorative fields; Design added the action to `/documents/signatures` and bound all five. Verified: changing the selects and saving issues `PUT /v1/esign/settings → 200`.
- **`SaveSignature` — CLOSED, engine side built.** Now carries the signature itself (`drawn`/`typed`/`uploaded`). Added `GET`/`PUT /v1/esign/signature` (DMS-09) storing it on the user rather than in settings — it is not configuration, it is them. Two deliberate refusals: the audit log records method, size and what was replaced but **never the specimen**, because a 5KB blob repeated on every change makes the chain unreadable and copies the signature into a second place; and a typed signature is refused when policy has `allowTyped` off, since a policy only the UI enforces is not a policy. Verified by drawing on the pad at `/account/signature`: `PUT → 200`, 6,684 bytes stored, audit entry `{bytes, method, replaced}`.
- **The fake-success default — FIXED AT SOURCE by Design.** `hook()` now routes the unregistered case through `fail()`. Our 29 honest-refusal stubs stay, because they name the specific consequence ("it is NOT on", "the delegate still holds it") rather than the generic line, but they are no longer the only thing standing between a missing seam and a false confirmation.
