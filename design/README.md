...
This bundle covers **Phase 1 and Phase 2**. Phase 2 added authentication and personal-settings screens, a workflow chain editor, the e-signature suite, printable documents, and bound the transaction detail page to its route parameter. `PHASE2-INTEGRATION.md` in this folder is the machine-facing manifest of every Phase 2 route, const and integration hook — read it alongside this document.
...
Open `WEWE ERP.dc.html` in a browser. `support.js` must sit beside it. Navigate by hash, e.g. `WEWE ERP.dc.html#/admin/roles`. A sign-in splash appears first — click through it to reach the app.
...
Brand accents used only on share buttons: WhatsApp `#25D366`, Facebook `#1877F2`, X `#1A1D21`.

### Dark mode

Toggled via a `data-wewe="dark"` attribute on the root, driven by a "Dark" control in the sidebar.
...
- Content is capped at `max-width:1320px` and left-aligned. Printable documents are 794px sheets and ignore that cap.
...
- **Procurement** — Vendor registry `/procurement` · Vendor register `/procurement/vendors` · RFQs `/procurement/rfq` · Purchase orders `/procurement/po` · Contracts `/procurement/contracts`
...
- **Documents** — Repository `/documents` · Viewer `/documents/view` · E-signature `/documents/esign` · Signature certificate `/documents/certificate` · External signer view `/sign/external` · Printable documents `/print/travel-authority` · Retention `/documents/retention`
...
- **Reports** — Library `/reports` · Custom builder `/reports/builder` · Pipeline analytics `/reports/pipeline` · Scheduled `/reports/scheduled`
- **Administration** — Users `/admin` · Roles & permissions `/admin/roles` · Forms & data capture `/admin/forms` · Form builder `/admin/forms/build` · User activity `/admin/activity` · Organisation structure `/admin/org` · Workflow configuration `/admin/workflow` · Approval chain editor `/admin/workflow/chain` · Reference data `/admin/reference` · Policy settings `/admin/policies`

**Personal routes** (reached from the profile menu, not the module nav): `/account/profile` · `/account/signature` · `/account/notifications` · `/account/delegation`
**Auth routes** (outside the shell in production): `/auth/2fa` · `/auth/reset` · `/auth/setup` · `/auth/locked`

### Top bar

Search field, spacer, **notification bell** (44px circle, unread count badge), **+ New** primary pill (44px, `border-radius:999px`), then the avatar + persona switcher. **+ New** opens a quick-create chooser of eight things, each routing to its real flow.

### Persona switching

Eight personas so reviewers can see role-scoped navigation; each hides the modules it has no rights to. In production navigation derives from the signed-in user's role.
...
**Stat card** — white, `border-radius:16px; padding:20px` + card shadow. 13px/500 `#8A8F98` label; 24px/600 value with tabular numerals; 12px `#9AA0A8` context line. Laid out `grid-template-columns: repeat(auto-fit, minmax(220px,1fr)); gap:12px`.
...
**Status pill** — `height:22px; padding:0 9px; border-radius:6px`, 10px/700 uppercase, tinted background + darker foreground. Where a tick or cross helps, an 11–12px stroked SVG sits inside at `stroke-width:2.6–3`.
...
**Wizard stepper** — clickable in both directions plus Back/Continue. Done steps show a tick, the current step pulses, future steps are outlined.

**Printable sheet** — `width:794px; padding:56px 64px` on white with the document shadow. Letterhead: 44px accent logo square, organisation name at 16px/700, address and registration at 10px, closed by a `3px solid #E0572E` rule. Verify QR at 78px top-right. Signature blocks are a 1px ink rule with 10px caption beneath.
...
- **`/requisitions/new`** — creation form, with the **over-budget** budget-check table and warn/block variants (see Behaviour).
- **`/requisitions/:ref`** — transaction detail, bound to the route parameter. Header carries title, status pill (`At <stage>` / Returned / Approved and closed / Rejected), ref, initiator, department, donor chip, an overdue chip when applicable, and the total with its budget line. Below it the five-stage tracker with per-stage actor, timestamp and elapsed time. Left column: line items with totals, then transaction history as an avatar timeline. Right column: the decision panel (Approve / Return for correction / Reject, shown only while pending, plus Open comments), a **Fulfilment** card on approved transactions, budget impact with a headroom bar, and documents.
- **`/requisitions/templates`** — 8 templates. Row actions **Use · Edit · Copy** open the template document with its real line items, typical total, and its **attached form**.
...
### Inventory & stores
- **`/inventory`** — 8 item lines across two stores. Level bar against the reorder point, red at or below. Row actions **Issue · Receive**. Banner flags T-shirts at zero and toner below reorder.
...
- **`/inventory/grn`** · **`/inventory/issues`** (row actions **Docket · Reverse**, or **Return · Docket** / **Return · Chase**) · **`/inventory/counts`**.
...
- **`/documents/esign`** — open signature requests with signatory order and what each is waiting on. Row actions **Sign · Certificate**.
- **`/documents/certificate`** — certificate of electronic signature. Letterhead, verify QR, document and execution blocks, signatory table (name, role, method, identity verified by, timestamp, IP), and the SHA-256 hash. A banner states **Hash matches** or **HASH MISMATCH — do not rely on this document**; *Re-verify hash* toggles between them.
...
- **`/hr/appraisals`** — weighted score (34px) and band in the header; four-stage sign-off tracker (self → supervisor → HR → filed). Tabs: *Objectives & ratings* (supervisor rating is an editable input per row; score, band and colour recalculate live — bands ≥4.5 Outstanding, ≥4 Exceeds, ≥3 Meets, ≥2 Needs improvement, else Unsatisfactory), *Competencies* (self `#6E9EC9` vs supervisor `#E0572E` as 5-segment bars, evidence per row, DISCUSSED flag on gaps), *Development plan*, *Cycle progress* across 47 staff.
...
- **`/timesheets/approvals`** — 6 submissions. Row **Approve** charges hours to grants and flips status; **Query** returns it; **Remind** for not-submitted. *Approve all compliant* clears the valid ones and holds the one below 100% effort.
- **`/timesheets/loe`**
...
- **`/reports/builder`** — four-step wizard: **Source & columns** (10 column chips, side panel showing output order, live count), **Filters** (dates, department, donor, a minimum-amount selector that genuinely re-filters — ₦250,000 → 21 of 24 rows), **Group & sort**, **Preview & save** (live preview from the chosen columns, amounts right-aligned, in a horizontally scrollable region with a 1160px minimum so all ten columns show without clipping the card). *Save and share* confirms it reached the library; *Schedule weekly* alongside.
...
- **`/admin/roles` — Roles & permissions.** 296px role list + content column.
  ...
  - **Header** — name, unpublished-changes pill, description, meta, and *Duplicate · View as… · Publish changes*.
...
- **`/admin/forms/build` — Form builder.** Three-column build view plus four tabs.
  - **Build** — 13-type field palette appending on click; a live preview where **every field renders as its real control**, each selectable (accent inset ring), reorderable and removable; right panel settings (label, help text, options, required toggle) plus form rules (partial answers, offline, one response per person, GPS).
  - **Mapping** — attach the form to any record: requisition template, retirement evidence, vendor register, staff file, findings register, asset campaign, or standalone. Explicit field-to-record mapping (*Actual spend → Retirement amount*; a safeguarding "Yes" opens a finding). Right rail lists who can respond.
  - **Distribution** — **In-app** (where it surfaces), **Email invitation** (recipients with group chips, subject, message, timing, reminders), **Public link** (link with Copy, WhatsApp / Facebook / X, QR download, close date, response limits, and a warning that sensitivity-set fields are blocked on public forms). Phone preview and an open → start → complete funnel alongside.
...
- **`/admin/workflow/chain` — Approval chain editor.** Type chips for five transaction types; five stage cards in a horizontal flow, each showing role, when it applies, and SLA, selectable to edit. Below, the stage editor: role select, `minAmountKobo` threshold, SLA hours, and a line stating whether the stage is conditional or always applies. Right rail: version history v1–v6 with the live one accented. Header actions *Test in sandbox* (walks a value through the draft chain showing which stages it visits and which are skipped) and *Save as version 7*. A standing banner states that changes apply to new transactions only.

### Authentication & personal

- **`/auth/setup`** — invite landing. Set password → confirm profile basics (name and department read-only, mobile and time zone editable) → account ready, straight into 2FA where the role requires it.
- **`/auth/2fa`** — three steps: scan the QR (with the hand-entry key, account and issuer, and a warning not to share it), confirm a six-digit code, then the one-time backup-code sheet (10 codes, Download / Print / Copy all, "I've saved these — finish").
- **`/auth/reset`** — email → check-your-email with a code field and resend timer → new password with a live policy checklist → confirmation stating other sessions were signed out.
- **`/auth/locked`** — three sign-in error states: wrong password with attempts remaining, temporary lock with a countdown and doubling delay, and administrator suspension (visually distinct, since waiting will not help).
- **`/account/profile`** — contact details (HR-owned fields greyed), active sessions table with per-row sign-out and *Sign out everywhere*, 2FA status card warning that only 3 backup codes remain, password age, saved-signature preview, and links onward.
- **`/account/signature`** — Draw / Type / Upload tabs; the draw pad is a real canvas with a signing line, Type offers two hands, Upload is a drop zone. Shows what is currently saved and where it has been used.
- **`/account/notifications`** — 9 categories × Instant / Daily digest / Off. Escalations and audit queries are marked ALWAYS INSTANT and locked, with an explainer on why they cannot be batched.
- **`/account/delegation`** — active-delegation banner with Cancel now; setup form with delegate picker, dates (14 of a 30-day maximum), scope and reason. Choosing yourself blocks on SOD-01; choosing someone already delegating onward blocks as a chain. The rules and past delegations sit alongside.

### Printables

- **`/print/travel-authority`** — TA-2026-0087 on letterhead: traveller, department, destination, dates, funding, linked transaction, purpose, an entitlements table totalling ₦796,000.00, the four-stage approval chain with names and dates, and two signature blocks.
- **`/print/purchase-order`** — PO-2026-0064: supplier and deliver-to blocks, requisition / funding / budget line / payment terms strip, numbered lines with subtotal, 7.5% VAT and total ₦4,798,800.00, five terms, an approval-evidence paragraph, and signature blocks for WEWE and supplier acceptance.
- **`/print/hr-letter`** — employment and salary confirmation addressed to a bank, with merge fields filled from the staff record, a staff-member selector above the sheet, signature block, and an issue-log line recording the reference, requester, addressee and the sensitivity set under which salary data was released.
...
|---|---|---|
| **Notification centre** | Bell | Right panel, 420px. "Needs my action" and "Updates" sections, unread dots, Mark all read, per-item deep link, footer link to preferences |
...
| **Bulk approve** | Queue → *Bulk approve* | Selected-items table with per-item eligibility, excluded rows greyed with reasons, running count and total value, one shared note, single confirm — then a results state (*n* approved, *m* skipped and why) |
...
| **Signing ceremony** | E-signature row → *Sign* | Three steps: full document review with the certification statement, method choice (saved / draw on canvas / type) with the field highlighted, then confirmation restating name, capacity, method, time, identity verification and IP |
| **Payslip** | Payslip row **View** | The actual payslip — letterhead, employee and payment block, earnings split (basic 50% / housing 25% / transport 15% / utility 10%), deductions (PAYE, pension 8%, NHF 2.5% of basic), net pay on an ink panel, employer contributions, and Jan–Aug year-to-date totals, all derived from the row |
...
| **Add a donor** | Grants | Three steps: donor identity → award (reference, value, period, reporting frequency, indirect rate, cost rules, agreement) → budget lines + manager. Opens as a draft until Finance confirms the budget reconciles |
...
- **Warn** — amber banner naming the count of offending lines and the total shortfall, per-line shortfall figures in the budget-check table, a **required** justification field, and a submit button that works. Copy notes over-budget requisitions always route to the Managing Director.
...
| File | What it is |
|---|---|
| `WEWE ERP.dc.html` | The complete design prototype — every screen, dialog and interaction described here |
| `PHASE2-INTEGRATION.md` | Phase 2 route list, data-const shapes, and the `window.__weweAct` contract |
