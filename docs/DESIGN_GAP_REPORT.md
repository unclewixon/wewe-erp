...
29. **The procurement module has one write control for the entire module.** The engine side is complete and tested end to end — vendors with dual-control bank details, blacklisting, RFQs, quotes, the quote-count threshold band with a sole-source path, award with a mandatory written justification, PO generation from the awarded RFQ, a printable PO, goods receipts, and contracts with payments and amendments. The bundle emits exactly one procurement hook, `SendRfq`, which is now wired. There is no control anywhere that adds a quote, awards a winning quote, raises the PO, records a goods receipt, registers or blacklists a vendor, or creates a contract — so none of that can be reached from the UI. Procurement reads are all live (vendor registry, RFQ comparison, purchase orders, contracts).
